...
for database in databases:
    try:
        ...
        asn_frequency_df = pd.read_sql_query(query, conn)

        # Calculate Gini coefficient for the country frequencies
        gini_coefficient = gini(asn_frequency_df['frequency'].values)

        # Store the result
        results.append({'database': database, 'asn_frequency_gini': gini_coefficient})
        print(f"Gini Coefficient of asn frequencies for {database}: {gini_coefficient}")

    except Exception as e:
        print(f"Error processing database {database}: {e}")
        results.append({'database': database, 'asn_frequency_gini': None})
...
output_filename = "../images/asn_frequency_gini_results.csv"
